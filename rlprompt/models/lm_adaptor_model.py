...
class LMAdaptorModel(BaseModel):
    ...
    def __init__(
        ...
        self.generator = pipeline(
            "text-generation", tokenizer=self.tokenizer, model=model, device=self.device
        )
        ...
        self.mlp = _build_one_layer_mlp(
            in_dim=model_dim, out_dim=model_dim, hidden_size=hidden_size
        ).to(self.device)
    ...
    def _mlp_forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        if self.fluent:
            ...
            logits = torch.where(
                lm_logits < min_values, torch.full_like(logits, float("-inf")), logits
            )

        return logits

    def teacher_forcing(
        self, source_texts: List[str], sample_ids: torch.Tensor, **kwargs
        ...
        for i in range(sample_ids.shape[-1]):
            ...
            tokens = [
                self.generator.tokenizer.convert_ids_to_tokens([a])[0]
                for a in actions.tolist()
            ]
            token_strs = [
                self.generator.tokenizer.convert_tokens_to_string([t]) for t in tokens
            ]
        ...
        output = dict(sample_logits=sample_logits, sample_ids=sample_ids)
        return output
    ...
    def sample(
        ...
        **kwargs
        ...
        sample_lengths = torch.tensor(
            [max_new_tokens for _ in range(sample_ids.shape[0])]
        ).to(self.device)
