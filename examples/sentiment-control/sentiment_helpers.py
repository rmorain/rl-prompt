...
import numpy as np
import pandas as pd
from sentiment_reward import PromptedClassificationReward, SentimentReward
from torch.utils.data import Dataset
from transformers import AutoTokenizer


class PromptedClassificationDataset(Dataset):
    def __init__(self, source_texts: List[str], class_labels: List[str]):
...
def load_few_shot_classification_dataset(
    ...
    return (source_texts, class_labels, num_classes, verbalizers, template)
...
def make_prompted_classification_reward(
    ...
    return PromptedClassificationReward(
        config.task_lm,
        config.is_mask_lm,
...
def make_sentiment_reward(config):
    return SentimentReward(config.task_lm, config)
